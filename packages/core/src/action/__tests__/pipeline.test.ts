...
describe('pipeline()', () => {
  ...
  it('halts on GuardFailedError and does not run subsequent actions', async () => {
    const ran: string[] = [];
    const fail = async () => {
      throw new GuardFailedError('blocked');
    };
    const after = async () => {
      ran.push('after');
    };

    await expect(pipeline(fail, after)(ctx)).rejects.toThrow(
      GuardFailedError,
    );
  ...
  it('passes ctx to each action', async () => {
    ...
    const capture = async (c: MenuContextLike) => {
      received.push(c);
    };
...
describe('guard()', () => {
  ...
  it('is composable inside pipeline', async () => {
    ...
    await expect(pipeline(checkAdmin, doWork)(ctx)).rejects.toThrow(
      'not admin',
    );
